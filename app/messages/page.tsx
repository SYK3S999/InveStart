...
export default function MessagesPage() {
  ...
  const handleSelectConversation = (conversation: SetStateAction<{ id: number; recipient: { id: number; name: string; avatar: string; role: string; project: string; lastSeen: string; isOnline: boolean }; lastMessage: { content: string; timestamp: string; isRead: boolean; sender: string }; unreadCount: number; isFavorite: boolean } | { id: number; recipient: { id: number; name: string; avatar: string; role: string; lastSeen: string; isOnline: boolean; project?: undefined }; lastMessage: { content: string; timestamp: string; isRead: boolean; sender: string }; unreadCount: number; isFavorite: boolean } | null>) => {
    setActiveConversation(conversation)
    setMessages(mockMessages) // In a real app, you would fetch messages for this conversation

    // Mark conversation as read
    if (!conversation || typeof conversation === "function") return;
  ...
  const formatConversationTime = (timestamp: string | number | Date) => {
    const date = new Date(timestamp)
    const now = new Date()
