...
export default function MessagesPage() {
  ...
  const handlePayFee = (conversationId: string, feeType: 'investorFeePaid' | 'startupFeePaid' | 'leasingFeePaid') => {
    setConversations((prev) => {
      return prev.map((conv) => {
        if (conv.id === conversationId) {
          const { officeFee, ...restConv } = conv;
          const updatedConv = {
            ...
            officeFee: undefined,
            unreadCount: 0,
            isFavorite: conv.isFavorite !== undefined ? conv.isFavorite : false,
            deal: conv.deal !== undefined
              ? { ...conv.deal, [feeType]: true }
              : {
                  amountDZD: 0,
                  equipment: [],
                  investorFeePaid: feeType === 'investorFeePaid',
                  startupFeePaid: feeType === 'startupFeePaid',
                  leasingFeePaid: feeType === 'leasingFeePaid',
                },
          };
          return updatedConv;
        }
                    ...
                    >
                      ...
                      <Tabs defaultValue="all" className="flex-1 flex flex-col">
                        <TabsList className="grid grid-cols-3 px-4 py-2 bg-gray-900">
                          <TabsTrigger value="all" className="data-[state=active]:bg-gray-700 data-[state=active]:text-blue-400">الكل</TabsTrigger>
                          <TabsTrigger value="unread" className="data-[state=active]:bg-gray-700 data-[state=active]:text-blue-400">غير مقروءة</TabsTrigger>
                          <TabsTrigger value="favorites" className="data-[state=active]:bg-gray-700 data-[state=active]:text-blue-400">المفضلة</TabsTrigger>
                        </TabsList>
                                ...
                                >
                                  <div className="p-4 relative flex items-start gap-3">
                                    <Avatar className="h-10 w-10">
                                      ...
                                      <AvatarFallback className="bg-gray-700 text-gray-100">{conv.recipient.name.charAt(0)}</AvatarFallback>
                                    </Avatar>
                                    <div className="flex-1 min-w-0">
                                      <div className="flex justify-between items-center">
                                        <h3 className="text-sm font-medium text-gray-100 truncate">{conv.recipient.name}</h3>
                                        <div className="flex items-center gap-2">
                                          <span className="text-xs text-gray-400">{formatConversationTime(conv.lastMessage.timestamp)}</span>
                                          <TooltipProvider>
                                            <Tooltip>
                                      ...
                                      {conv.recipient.project && (
                                        <p className="text-xs text-blue-500 mb-1">{conv.recipient.project} ({conv.recipient.wilaya})</p>
                                      )}
                                      ...
                                      {(conv.unreadCount ?? 0) > 0 && (
                                        <Badge className="absolute top-4 left-4 bg-blue-500 text-white">{conv.unreadCount ?? 0}</Badge>
                                      )}
                                    </div>
                                  </div>
                                </motion.li>
                                  ...
                                  >
                                    <div className="p-4 relative flex items-start gap-3">
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <Badge className="absolute top-5 left-4 bg-blue-500 text-white">{conv.unreadCount}</Badge>
                                      </div>
                                    </div>
                                  </motion.li>
                                ))}
                                  ...
                                  >
                                    <div className="p-4 relative flex items-start gap-3">
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-sm text-gray-400 truncate">{conv.lastMessage.content}</p>
                                        {(conv.unreadCount ?? 0) > 0 && (
                                          <Badge className="absolute top-4 left-4 bg-blue-500 text-white">{conv.unreadCount}</Badge>
                                        )}
                                      </div>
                                    </div>
                                  </motion.li>
                ...
                <div className="w-full md:w-2/3 lg:w-3/4 flex flex-col">
                  ...
                  {activeConversation ? (
                    <>
                      <div className="p-4 border-b border-gray-700 bg-gray-800 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-10 w-10">
                            ...
                            <AvatarFallback className="bg-gray-700 text-gray-100">{activeConversation.recipient.name.charAt(0)}</AvatarFallback>
                          </Avatar>
                          <div>
                            <h3 className="text-sm font-medium text-gray-100">{activeConversation.recipient.name}</h3>
                         ...
                         oportunity
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button variant="ghost" size="icon" className="text-gray-400 hover:text-blue-400 rounded-full">
                            ...
                            <DropdownMenuContent align="end" className="bg-gray-800 text-gray-100 border-gray-700">
                              ...
                              <DropdownMenuItem className="text-red-400">حذف المحادثة</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </div>
                      </div>
                      ...
                      <div className="p-4 bg-gray-900 border-b border-gray-700">
                        ...
                        <div className="space-y-2">
                          <p className="text-sm text-gray-300">قيمة الصفقة: {formatDZD(activeConversation.deal?.amountDZD ?? 0)}</p>
                          <div>
                          ...
                          <div className="flex gap-2">
                            {user?.role === "sponsor" && activeConversation.deal && !activeConversation.deal.investorFeePaid && (
                              <Button
                                onClick={() => handlePayFee(activeConversation.id, 'investorFeePaid')}
                                className="bg-blue-500 text-white hover:bg-blue-600"
                              >
                                دفع رسوم المستثمر ({formatDZD(calculateFees(activeConversation.deal.amountDZD).investorFee)})
                              </Button>
                            )}
                            {user?.role === "startup" && activeConversation.deal && !activeConversation.deal.startupFeePaid && (
                              <Button
                                onClick={() => handlePayFee(activeConversation.id, 'startupFeePaid')}
                                className="bg-blue-500 text-white hover:bg-blue-600"
                              >
                                دفع رسوم صاحب المشروع ({formatDZD(calculateFees(activeConversation.deal.amountDZD).startupFee)})
                              </Button>
                            )}
                            {user?.role === "startup" && activeConversation.deal && !activeConversation.deal.leasingFeePaid && (
                              <Button
                                onClick={() => handlePayFee(activeConversation.id, 'leasingFeePaid')}
                                className="bg-green-500 text-white hover:bg-green-600"
                              >
                                دفع رسوم التمويل التأجيري ({formatDZD(calculateFees(activeConversation.deal?.amountDZD ?? 0).leasingFee)})
                              </Button>
                            )}
                          </div>
                          {(activeConversation.deal?.investorFeePaid || activeConversation.deal?.startupFeePaid || activeConversation.deal?.leasingFeePaid) && (
                            <p className="text-sm text-green-400">
                              {activeConversation.deal?.investorFeePaid && "تم دفع رسوم المستثمر | "}
                              {activeConversation.deal?.startupFeePaid && "تم دفع رسوم صاحب المشروع | "}
                              {activeConversation.deal?.leasingFeePaid && "تم دفع رسوم التمويل التأجيري"}
                            </p>
                          )}
                        </div>
                      </div>
                                ...
                                {msg.attachment && (
                                  <div className="mb-2 p-2 bg-gray-700 rounded flex items-center">
                                    <div>
                                      ...
                                      <div>
                                        <p className="text-sm font-medium text-gray-100">{msg.attachment.name}</p>
                                        <p className="text-xs text-gray-400">{msg.attachment.size}</p>
                                      </div>
                                      <Button variant="ghost" size="sm" className="ml-auto mr-2 text-gray-400 hover:bg-gray-600">
                                ...
                                <div className="mt-1 flex items-center justify-end gap-2">
                                  ...
                                  {msg.sender === "me" && (
                                    <span className="flex items-center">
                                      ...
                                      {msg.status === "delivered" && <Check className="h-3 w-4 text-blue-400" />}
                                      {msg.status === "read" && (
                      ...
                      <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button type="button" variant="ghost" size="icon" className="text-gray-400 hover:bg-gray-600 rounded-full">
                    ...
                    <div className="flex flex-col items-center justify-center h-full p-4">
                      ...
                      <Button asChild className="bg-blue-500 text-white rounded-full hover:bg-blue-600">
